...
async function loadOrder() {
  try {
    order.value = await request(`/shop/orders/${route.params.id}`)
...
async function saveStatus() {
  ...
  try {
    order.value = await request(`/shop/orders/${order.value.id}/status`, {
      method: "PUT",
...
async function downloadInvoice() {
  ...
  try {
    await nextTick()
    await downloadElementAsPdf("admin-order-invoice-download", buildFileName())
  } catch (error: any) {
    status.value = error.message
  } finally {
...
<template>
  <div>
    <PageHeader
      ...
      :subtitle="t('order.details_subtitle_admin')"
    />

    <div class="page-actions">
      <NuxtLink to="/" class="link-btn">{{ t("order.back_to_dashboard") }}</NuxtLink>
      <NuxtLink v-if="order?.customer_id" :to="`/customers/${order.customer_id}`" class="link-btn">
    ...
    <section v-if="order" class="panel" style="margin-top: 14px">
      ...
      <div class="toolbar">
        ...
        <button type="button" :disabled="saving" @click="saveStatus">
          {{ saving ? t("auth.loading") : t("order.save_status") }}
        </button>
      </div>
    ...
    <div v-if="order?.invoice_ready" class="pdf-capture-stage" aria-hidden="true">
      <OrderInvoiceSheet
        ...
        sheet-id="admin-order-invoice-download"
      />
    </div>
  </div>
</template>
